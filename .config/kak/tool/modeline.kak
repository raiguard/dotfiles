...
%{
    evaluate-commands %sh{
        ...
        if [[ $kak_client == "tools" || $kak_client == "docs" ]]; then
            ...
            lsp_section=$( [ "$kak_opt_lsp_enabled" = true ] && printf "{comment}%%opt{lsp_modeline_progress}  {InlayDiagnosticError}%%opt{lsp_diagnostic_line_error_sign}%%opt{lsp_diagnostic_error_count} {InlayDiagnosticWarning}%%opt{lsp_diagnostic_line_warning_sign}%%opt{lsp_diagnostic_warning_count}  " )
            filetype_section=$( [ -n "$kak_opt_filetype" ] && printf "{InlayDiagnosticWarning}%%opt{filetype} " )

            echo "set window modelinefmt \"$lsp_section{string}%%opt{modeline_git_branch}{StatusLineValue}%%val{bufname}{StatusLineValue}:%%val{cursor_line}:%%val{cursor_char_column}{StatusLine}{{context_info}} $filetype_section{{mode_info}} {keyword}%%val{client}{StatusLineValue} {enum}%%val{session}{StatusLine}\""
        fi
    }
}
